function [fragments, junctions, neighbor_data, seg, avg_seg_colors, ...
    polyfragments, poly_params] = seg2fragments(seg, img, min_area, order,cm)
%
...
jan = 1;
while (any(seg(:)==0))
    %figure(7); imshow(label2rgb(seg,cm));
    %imwrite(label2rgb(seg,cm),strcat(num2str(jan),'.jpg'));
    %pause;
    seg = fill_in_segmentation(img, seg, 0, 8);
    jan = jan +1;
end
